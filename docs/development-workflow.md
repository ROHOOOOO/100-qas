...
- 文件命名清晰，避免无意义缩写。
- 用户可见文案使用中文。
- 新增小游戏时，先更新 `docs/product-requirements.md` 和对应游戏需求文档，再实现入口和核心流程。
- 不同小游戏应使用独立路由和独立数据模型，避免互相影响。
...
- 加入第二位本地 Tycoon 玩家。
- 房主开始游戏，玩家轮流掷骰；买地/升级后自动换人，超时默认跳过。
...
- 再次确认提交后进入结果页。
- 提交后没有可编辑输入框。
- 结果页展示当前题库数量对应的题目卡片和答案。
- 可新增第二个本地玩家。
- 未提交玩家不能查看结果。
- 可切回已提交玩家查看结果。
- 本地存储中玩家状态与答案数量正确。
- 本地存储中房间保存了自定义题库。
- 旧版 `#room/:code` 路由仍能打开房间。
